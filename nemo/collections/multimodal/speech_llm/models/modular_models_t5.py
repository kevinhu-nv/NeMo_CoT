...
class ModularizedAudioT5Model(MegatronT5LoraModel):
    ...
    def forward(
        ...
        # print(f'audio_batch: {audio_batch}')
        has_hyp = audio_batch['has_hyp']
        print(f'has_hyp: {has_hyp}')
        def generate_loss_mask(dec_input, dec_mask, sep_id=2):
            batch_size, seq_len = dec_input.size()
            for i in range(batch_size):
                target_indices = (dec_input[i] == sep_id).nonzero(as_tuple=True)[0]
                if len(target_indices) >= 2:
                    second_target_index = target_indices[1].item()
                else:
                    second_target_index = 0
                dec_mask[i, :second_target_index] = 0

            return dec_mask
        print(f'dec_input: {dec_input}')

        # Only mask the ASR transcript when it is the a hyp not gt
        if has_hyp.any():
            loss_mask = generate_loss_mask(dec_input, dec_mask, sep_id=2)
        else:
            loss_mask = dec_mask

        print(f'self.tokenizer.pad_id: {self.tokenizer.pad_id}')
