...
class TextProcessing:
    ...
    def _process_example(self, context: str, output: str, lang: str):
        ...
        print(f'answer_text: {answer_text}, tokenization lang: {lang}')

        # Hack(huk): Separate by <sep> and insert a sep_id in between the two segments
        has_asr_hyp = True
        if 'sep_gt' in answer_text:
            answers = answer_text.split('<sep_gt>')
            has_asr_hyp = False
        elif 'sep_hyp' in answer_text:
            answers = answer_text.split('<sep_hyp>')
        else:
            answers = answer_text
        
        # If false, put the src_text to context
        add_src_text_to_tgt = False
        print(f'has_asr_hyp: {has_asr_hyp}')
        ...
        if len(answers) == 1:
            ...
            if add_src_text_to_tgt:
                # Add ASR gt
                asr_text_ids = _text_to_ids(
                    answers[0], self.sample_alpha if self.input_text_mask_ratio is None else None, lang=lang
                )
                answer_ids = pre_pad + asr_text_ids
                asr_text_len = len(asr_text_ids)
                print(f'asr_text_len: {asr_text_len}')
                # Add sep_id
                answer_ids.append(self.sep_id)
                # Add AST gt
                answer_ids += _text_to_ids(
                    answers[1], self.sample_alpha if self.input_text_mask_ratio is None else None, lang=lang
                )
            else:
                print(f'add_src_text_to_tgt: {add_src_text_to_tgt}')
                answer_ids = pre_pad + _text_to_ids(
                    answers[1], self.sample_alpha if self.input_text_mask_ratio is None else None, lang=lang)
                context_with_src_text = original_context + '. The source text is: ' + answers[0]
                context = (
                    self.prompt_template.replace(f'{{{self.input_key}}}', context_with_src_text)
                    .replace(f'{{{self.output_key}}}', '')
                    .strip(' ')
                )
        ...
        print(f'context: {context}, tokenization lang: {lang}')
        context_ids = pre_pad + _text_to_ids(context, lang=lang)
        print(f'context_ids: {context_ids}')
        print(f'add_sep: {self.add_sep}, sep_id: {self.sep_id}')
        ...
        if total_ids > self.max_seq_length:
            print(f'truncation: total_ids: {total_ids}, self.max_seq_length: {self.max_seq_length}')
        ...
        # if has_asr_hyp:
        #     answer_start_idx += asr_text_len
        #     print(f'answer_start_idx: {answer_start_idx}')
        ...
        processed_example = {
            ...
            'has_hyp': torch.as_tensor(has_asr_hyp),
        }
...
def convert_canary_prompt_to_text(prompt, is_canary_tokens_augment):
    ...
    print(f'convert_canary_prompt_to_text, ps: {ps}')
    ...
    if len(ps) == 6:
        ...
        pnc = get_pnc(ps[4])
        print(f'is_canary_tokens_augment: {is_canary_tokens_augment}')
    ...
    print(f'convert_canary_prompt_to_text, task: {task}')
    return task
...
class LhotseAudioQuestionAnswerDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, cuts) -> dict[str, torch.Tensor | list[str] | dict]:
        ...
        has_hyp = return_batch['has_hyp']
        print(f'LhotseAudioQuestionAnswerDataset --> has_hyp: {has_hyp}')
        print(f'return_batch keys: {return_batch.keys()}')
...
def collate_text_data(
    ...
    has_hyp = [x for x in fields['has_hyp']]
    print(f'collate_text_data --> has_hyp: {has_hyp}')
    ...
    return {
        ...
        "has_hyp": torch.as_tensor(fields["has_hyp"]),
    }
