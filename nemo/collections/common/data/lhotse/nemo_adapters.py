...
class LazyNeMoIterator:
    ...
    def __iter__(self) -> Generator[Cut, None, None]:
        for data in self.source:
            ...
            # Get gt or hyp ASR text
            print(f'data: {data}')
            if 'hyp' in data:
                text_actual = data.get('hyp')
            else:
                text_actual = data.get('text')
            print(f'ASR text: {text_actual}')    
            text = data.get(self.text_field)
            print(f'answer: {text}')
            # CoT: Concatenate source language text with target language text
            # TODO(huk): Choose the right separator
            if 'hyp' in data:
                text_cot = text_actual + " <sep_hyp> " + text
            else:
                text_cot = text_actual + " <sep_gt> " + text
            print(f'text_cot: {text_cot}')
            print(f'lang_field: {data.get(self.lang_field)}')
        ...
        for sid in shard_ids:
            ...
            with tarfile.open(fileobj=open_best(tar_path, mode="rb"), mode="r|*") as tar:
                for data, tar_info in zip(shard_manifest, tar):
                    ...
                    print(f'answer: {text}')
                    text_actual = data.get('text')
                    print(f'text: {text_actual}')
                    # CoT: Concatenate source language text with target language text
                    # TODO(huk): Choose the right separator
                    text_cot = text_actual + " <sep> " + text
                    print(f'text_cot: {text_cot}')
                    print(f'lang_field: {data.get(self.lang_field)}')
