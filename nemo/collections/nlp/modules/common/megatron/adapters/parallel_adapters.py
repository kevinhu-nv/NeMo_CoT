...
class ParallelLinearAdapter(nn.Module, AdapterModuleUtil):
    def __init__(
        ...
        **kwargs,
        ...
        print(f'ParallelLinearAdapter (LoRA), in_features: {in_features}')
        print(f'ParallelLinearAdapter (LoRA), dim: {self.dim}')
        print(f'ParallelLinearAdapter (LoRA), out_features: {out_features}')
    ...
    def forward(self, x):
        ...
        # print(f'ParallelLinearAdapter x_in_in: {x.shape}')
        x, _ = self.linear_in(x)  # (@adithyare) ColumnLinear returns output and bias, we are ignoring the bias term.
        # print(f'ParallelLinearAdapter x_in_out: {x.shape}')
        x = self.activation(x)
        x, _ = self.linear_out(x)
        # print(f'ParallelLinearAdapter x_out_out: {x.shape}')
