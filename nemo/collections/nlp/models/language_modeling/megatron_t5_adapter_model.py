...
class MegatronT5LoraModel(MegatronT5BaseAdapterModel):
    ...
    def __init__(self, cfg: DictConfig, trainer: Trainer):
        ...
        def detailed_summary(module, target_depth=6):
            total_params = 0
            trainable_params = 0
            non_trainable_params = 0

            def print_layer_summary(prefix, submodule):
                nonlocal total_params, trainable_params, non_trainable_params
                num_params = sum(p.numel() for p in submodule.parameters())
                num_trainable_params = sum(p.numel() for p in submodule.parameters() if p.requires_grad)
                num_non_trainable_params = num_params - num_trainable_params
                
                print(f"| {prefix:<20} | {submodule.__class__.__name__:<40} | {num_params/1e6:.2f}M")
                
                total_params += num_params
                trainable_params += num_trainable_params
                non_trainable_params += num_non_trainable_params

            def traverse_and_print(prefix, submodule, depth, target_depth):
                if depth == target_depth:
                    print_layer_summary(prefix, submodule)
                else:
                    for name, child in submodule.named_children():
                        traverse_and_print(f"{prefix}.{name}", child, depth + 1, target_depth)

            # Traverse to a specific depth (e.g., 4 levels deep)
            traverse_and_print("frozen_model", module, 0, target_depth)
        ...
        detailed_summary(self.frozen_model, target_depth=6)        
